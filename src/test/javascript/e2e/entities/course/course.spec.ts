...
describe('Course e2e test', () => {
  ...
  it('should create and save Courses', async () => {
    ...
    await promise.all([
      courseUpdatePage.setIDcourseInput('5'),
      courseUpdatePage.setIddelverymanInput('5'),
      courseUpdatePage.deliveryManSelectLastOption()
    ]);

    expect(await courseUpdatePage.getIDcourseInput()).to.eq('5', 'Expected iDcourse value to be equals to 5');
    expect(await courseUpdatePage.getIddelverymanInput()).to.eq('5', 'Expected iddelveryman value to be equals to 5');
