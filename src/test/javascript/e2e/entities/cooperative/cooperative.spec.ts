...
describe('Cooperative e2e test', () => {
  ...
  it('should create and save Cooperatives', async () => {
    ...
    await promise.all([
      ...
      cooperativeUpdatePage.setSurnameInput('surname'),
      cooperativeUpdatePage.setTelephoneInput('telephone'),
      cooperativeUpdatePage.setAddressInput('address')
    ]);

    expect(await cooperativeUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    expect(await cooperativeUpdatePage.getSurnameInput()).to.eq('surname', 'Expected Surname value to be equals to surname');
    expect(await cooperativeUpdatePage.getTelephoneInput()).to.eq('telephone', 'Expected Telephone value to be equals to telephone');
